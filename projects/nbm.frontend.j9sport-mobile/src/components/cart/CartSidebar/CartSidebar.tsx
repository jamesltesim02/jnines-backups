      ...
      if (SingleBetStore.record.length > 0 || ComboBetStore.comboQueue.length > 0) {
        queryArr = [
          ...SingleBetStore.record.filter(
            ({ticketId, betTime, status}) => (
              ticketId
              && betTime
              && status === undefined
              && Date.now() - betTime > 5000
            )
          ),
          ...
          pull.getCheckTickets(queryArr.join(',')).then((res: any[]) => {
            res.forEach((item: any) => {
              ...
              if ([1, 3].includes(item.ticketStatus)) {
                if (comboIndex !== -1) {
                  ComboBetStore.receiveTicketStatus(item.ticketId, TICKET_STATUS.SUCCESS, intl.formatMessage({id: 'ticket.status_1'}), item.opts)
                }
                if (singleIndex !== -1) {
                  SingleBetStore.receiveTicketStatus(item.ticketId, TICKET_STATUS.SUCCESS, intl.formatMessage({id: 'ticket.status_1'}), item.opts)
                }
              }
              // 状态11，4失败
              if ([11, 4].includes(item.ticketStatus)) {
                if (comboIndex !== -1) {
                  ComboBetStore.receiveTicketStatus(item.ticketId, TICKET_STATUS.FAILED, intl.formatMessage({id: 'ticket.status_0'}), item.opts)
                }
                if (singleIndex !== -1) {
                  SingleBetStore.receiveTicketStatus(item.ticketId, TICKET_STATUS.FAILED, intl.formatMessage({id: 'ticket.status_0'}), item.opts)
                }
              }
            })
          }).finally(() => {
