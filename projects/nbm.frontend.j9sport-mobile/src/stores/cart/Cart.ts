...
class Cart {
  ...
  onDataChange(event: PushEvent) {
    ...
    if (event.nt === 100) {
      ...
      if (single) {
        SingleBetStore.receiveTicketStatus(single.ticketId, status, errMsg, event.data?.opts[0])
      }
      // 串关推送
      const combo = ComboBetStore.comboQueue.find(val => val.ticketId === event.data?.ticketId)
      if (combo) {
        ComboBetStore.receiveTicketStatus(combo.ticketId as string, status, errMsg, event.data?.opts)
      }
    }
    ...
    if (event.nt === 2) {
      ...
      event.data?.mks?.forEach((market: { ops: any[], betbar: string}) => {
        market.ops.forEach(opt => {
          // 单式
          if (opt.oid === SingleBetStore.current.optionId) {
            // 单式串关bst为0或-1删除
            if (opt.bst === 0 || opt.bst === -1) {
              this.toggleSidebar()
              SingleBetStore.removeSingleBet()
            }
            // 更新到单式
