...
class Pusher {
  ...
  _receiveMessage ({ data }) {
    ...
    // this._log(`-----received[${msg.action || ''}:${msg.nt || ''}](${msg.data && msg.data.mid ? msg.data.mid : ''}):`, msg, this.userId)
    if (!msg) {
      return
    }
  ...
  _createWebSocket () {
    try {
      ...
      const ws = this.ws = new WebSocket(
        `${
          window.__AGYY_SPORTS_CONFIG__.PUSH_URL
        }${
          this.userId
        }`
      )
