...
const type = types.model(
  ...
  get pushData() {
        ...
        matchs: [
          {
            ...
            matchIds: [
              ...new Set([
                self.detail.matchId,
                ...self.normal.list.map(match => match.matchId),
                ...self.featured.list.map(match => match.matchId),
              ])
            ]
          },
        ],
        mainMatchId: self.detail.matchId
      }
    }
    ...
    return {
      matchs: availableSports.map(sport => {
   ...
   * 清空所有数据
   *
   * @param {string} key 被清空的列表名
   * @param {Boolean} clearDetail 是否同时清空详情
   */
  clear (key, clearDetail = true) {
    if (key) {
      self[key].clear()
      return
    }
  ...
  onDetailDataChange (event) {
    if (![event.data.matchId, event.data.mid].includes(self.detail.matchId)) {
      return
    }
...
export default {
  ...
  initial: () => {
    ...
    instance.init()
    window.__TEST_PUSH__ = instance.onDataChange.bind(instance)
    return instance
  }
}
