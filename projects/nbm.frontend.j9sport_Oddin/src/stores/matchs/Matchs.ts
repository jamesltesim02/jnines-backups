...
import { AVAILABLE_SPORTS, LIST_MARKETS, MarketStage, MatchState, PushNt, SportType } from '../../consts/match';
...
class MatchStore {
  ...
  /** 详情 */
  private _detail: Match | null = null;
  /** 进球提示声队列 */
  ...
  list (key: string): Array<Match> {
    const listObj = this.matchMap.get(key);
    if (!listObj) {
      return [];
    }
  ...
  private listBySport (
    callback: (sportId: SportType, list: Array<Match>) => void
  ) {
