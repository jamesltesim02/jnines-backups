...
import { SportType, LIST_MARKETS, AVAILABLE_SPORTS, MatchState, Sports } from '../consts/match';
...
export default class Pull extends BaseApi {
   ...
   * 获取当前系统状态
   *
   * *** 此接口由运维在后端nginx中实现,不是真正由pull服务实现
   *
   * @return
   *  {  
  ...
  /** 查询首页焦点比赛 */
  getHomeMatchs(sports: Array<SportType>) {
    return this._post(
      'home/getHomeMatchs',
  ...
  getUserBetStatistics(
    ...
    return this._get(
      '/ticket/getUserBetStatisticsForPlatform',
      {
        params: {
          dayCount,
          userName
  ...
  /** 查询即将开赛 */
  getComingSoon(sportId: SportType) {
    return this._get(
      'right/getReommMatch',
      {
        params: {
