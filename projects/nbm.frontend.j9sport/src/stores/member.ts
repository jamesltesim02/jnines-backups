...
class MemberStore {
  ...
  /** 控制登录注册忘记密码弹窗*/
  private _accountModal: {
    visible?: boolean
    type?: 'login' | 'forget' | 'register'
  } = {
    visible: false,
    type: 'login'
  }
  ...
  get balance() {
    ...
    return +Number(this._j9Balance?.targetBalance || 0).toFixed(2);
  }
  ...
  @computed
  get currency(): 1|2 {
    return this._info?.currency || 2;
  }

  get currencyMark() {
    return ` (${CURRENCY_MAP[this.currency]})`
  }
  ...
  /** 登录弹窗 */
  goLogin() {
    this._accountModal = {visible: true, type: 'login'}
  }

  /** 忘记密码弹窗 */
  goForget() {
    this._accountModal = {visible: true, type: 'forget'}
  }

  /** 注册弹窗 */
  goRegister() {
    this._accountModal = {visible: true, type: 'register'}
  }
}
