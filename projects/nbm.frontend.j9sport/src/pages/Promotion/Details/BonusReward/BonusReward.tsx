...
function BonusReward() {
  ...
  useEffect(() => {
    if (willGetting !== 0) {
      ...
      const tWidth = document.getElementsByClassName('progress-bar-strip')[0].clientWidth
      const pWidth = Array.from(stripEls)
        .slice(0, amountIndex + 1)
        .reduce((prev, acc, index) => {
          if (index === amountIndex) {
            return prev + acc.offsetWidth * progress
          }
          return prev + acc.offsetWidth
        }, 0)
      setProgressWidth(Math.round(pWidth / tWidth * 10000) / 100 + "%")
    }
  }, [amount])
