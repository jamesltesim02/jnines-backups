...
function J9Ticket(
	...
	useEffect(() => {
		...
		// 查询注单
		pull.getTickets({...queryParams, userName: memberStore.username}).then(({tickets,ticketCount }) => {
			...
			<div className="ticket-content">
				...
				<div className="ticket-content-header">
					...
					<div className="settle">
						...
						{
							SETTLE_STATES.map(state => (
								<span
									key={state}
									className={queryParams.ticketState === state ? "selected": ''}
									onClick={()=>{
										setQueryParams({
											...queryParams,
											ticketState: state,
						...
						<div className="query-time">
								<RangePicker
									...
									onChange={(date: any) => {
										setFilterParams({
											...filterParams,
											startTime: dayjs(date[0].format('YYYY-MM-DD 00:00:00')).valueOf(),
											limitTime: dayjs(date[1].format('YYYY-MM-DD 23:59:59')).valueOf()
										})
									}}
								/>
						</div>
...
export default withApi({pull: Pull})(observer(J9Ticket));