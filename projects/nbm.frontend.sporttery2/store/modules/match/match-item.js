...
const type = types.model(
  'MatchItem',
  {
    ...
    /** 比赛开始时间(时间戳) */
    matchDate: types.number,
    /** 竞彩id */
    lotteryId: types.maybeNull(types.string),
    /** 主队logo */
    ...
    /** 联赛logo */
    tournamentLogo: types.maybeNull(types.string),
    /** 数据统计 */
  ...
  get categories () {
    ...
    self.markets.forEach(market => {
      ...
      if (self.sportId === 10) {
        if ([1, 2].includes(market.marketStage)) {
          category = 3
        }
  ...
  onItemDataChange (event) {
    console.log('item data change:', self.matchName)
