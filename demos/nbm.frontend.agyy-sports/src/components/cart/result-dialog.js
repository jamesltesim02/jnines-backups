...
const ResultDialog = ({
  ...
  return (
    ...
    >
      <header
        className={`${classes.header} ${classes[state]}`}
      >
        <h3>
          <M id={`bet.${state}`} />
          {
      ...
      <footer className={classes.footer}>
        <div className={classes.amount}>
          <span><M id="bet.totalAmount" />: {amount}</span>
          <span>
            <M id="bet.totalBack" />: {Number((amount * cart.odds) || 0).toFixed(2)}
            </span>
        </div>
