...
const SigninPage = ({
  ...
  const doSignin = async () => {
    ...
    try {
      ...
      toast.success(intl.formatMessage({ id: 'others.signOK' }))
      window.location = `/?token=${result.token}`
    } catch (e) {
      if ([10010, 10086].includes(e.code)) {
        toast.error(intl.formatMessage({ id: 'others.accountNotMatch' }))
      }
    } finally {
      toast.loading(false)
    }
  ...
  return (
    <SubPage
      navProps={{
        titleKey: 'others.signTitle',
        links: [
          { to: '/', textKey: 'common.home' },
          { textKey: 'others.signTitle' }
        ]
      }}
