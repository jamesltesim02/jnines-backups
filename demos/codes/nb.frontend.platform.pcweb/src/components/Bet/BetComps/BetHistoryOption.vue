  ...
  <div class="nb-history-option">
    <div :class="boxClass" v-if="!betCheck" >
      <div class="history-body-item flex-start">
        <span class="head-index flex-center" v-if="showId || !isSingle" :style="{ color: oddsColor }">{{data.idx}}</span>
        <span class="body-item-team">{{$t(`common.sports.${data.sno}`)}}</span>
        <div class="body-item-line flex-center"></div>
        <span class="body-item-legal">{{data.tn || data.tournamentName || ''}}</span>
        <div class="body-item-line flex-center"></div>
        <rolling-text class="body-item-team" :text="getCompName(data)" :maxLength="53" scrollamount="2" />
      ...
      <div class="detail-real-head flex-start">
        ...
        <span class="head-blank"></span>
        <span class="head-team">{{getOptName(data)}}</span>
        <span class="head-odds">{{getOddv(data)}}</span>
        <span class="head-blank"></span>
        <span class="head-blank"></span>
        <div class="head-win-box" v-if="!isSingle">
          <span class="head-box-win flex-center" v-if="/^\d{2,3}$/.test(data.res)" :style="winStyle">{{data.winStu}}</span>
          <span class="head-box-lose flex-center" v-else-if="/^-\d{2,3}$/.test(data.res)" :style="loseStyle">{{data.winStu}}</span>
          <span class="head-box-other flex-center" v-else>{{data.winStu || ''}}</span>
        </div>
      </div>
    </div>
    <div class="history-body-mix" v-else >
      <div class="detail-real-head flex-start">
      ...
      <div class="history-body-item flex-start">
        ...
        <span class="mix-comp-rst flex-center" v-if="data.fmsc && data.mid && needShow" @click="toResultPage(data)">
          {{$t('pageBet.resultBtn')}}
        </span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState({ bFlag: state => state.bet.betHistoryFlag }),
    needShow() {
...
.nb-history-option {
  width: 100%;
  .history-body-item { width: 100%; height: 20px; padding: 0 20px; }
  .history-body-single, .history-body-multiple { .history-body-item { margin: 10px 0; } span, marquee { white-space: nowrap; font-size: 14px; user-select: none; } }
  .history-body-mix .history-body-item { margin: 2px 0; }
  .body-item-legal { max-width: 140px; }
  .body-item-team { max-width: 320px; font-size: 14px; }
  .body-item-line { width: 9px; height: 14px; margin-right: 8px; }
  .head-index { font-size: 13px; padding-top: 1px; padding-right: 8px; }
  .detail-real-head span { line-height: 100%; }
  ...
  .detail-real-head { width: 100%; height: 26px; padding: 0 20px; }
  ...
  .head-win-box span { padding: 0 4px; border-radius: 2px; font-size: 12px; height: 22px; line-height: 22px; user-select: none; }
  .head-team, .head-odds { font-size: 16px; }
...
.white .nb-history-option {
  ...
  .history-body-item span, .history-body-item marquee, .body-item-team { color: #3A3A3A; }
