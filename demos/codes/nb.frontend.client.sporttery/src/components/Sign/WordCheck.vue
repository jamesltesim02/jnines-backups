<template>
  <div class="word-check">
    <div class="check-box" :style="`bottom: ${btnClick ? '-.3rem' : '0'}`">
      <v-touch class="img-box" v-if="btnClick" @tap="imgBoxFun">
        <img class="img-picture" :src="captchaData" />
      ...
      <v-touch :class="`check-btn${pass ? '-active' : ''} flex-center`" @tap="checkBtnFun">
    ...
    async imgBoxFun() {
      ...
      if (x >= 0 && x <= ntWidth && y >= 0 && y <= ntHeight) {
        ...
        if (this.clickArr.length === this.textArr.length) {
          try {
            const textCaptcha = btoa(JSON.stringify(this.clickArr));
            await checkingWords({ type: this.type, code: textCaptcha });
            this.btnClick = false;
            this.$emit('update:textCaptcha', textCaptcha);
