  ...
  <bet-cover-box class="nb-bet-jump-page" :index="399999" :show="show" @close="closeFun">
    <div class="jump-page-body">
      ...
      <div class="body-btns flex-center">
        ...
        <v-touch class="body-btn-submit flex-center" :style="{ background: btnColor }" @tap="jumpFun">{{data && data.btn ? data.btn : ''}}</v-touch>
      </div>
    </div>
...
export default {
  ...
  name: 'BetJumpPage',
  computed: {
    ...mapState('app', { data: state => state.jumpObj, theme: state => state.theme }),
    show() {
  ...
  methods: {
    ...mapMutations('app', ['setJumpObject', 'setJumpStatus']),
    closeFun() {
      this.setJumpObject();
    },
    jumpFun() {
...
.nb-bet-jump-page .nb-bet-public-box-body {
  ...
  .jump-page-body {
    ...
    .body-alerts {
      width: 100%;
      height: 1.66rem;
      .body-blank { width: 100%; height: .98rem; }
      .title-box {
        width: 100%;
        height: .68rem;
        .body-title-big {
