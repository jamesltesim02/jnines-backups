<template>
  <div class="date-select flex-around">
    <v-touch class="select-from flex-center" @tap="showFrom">
    ...
    <v-touch class="select-to flex-center" @tap="showTo">
...
export default {
  ...
  data() {
    return {
      ...
      fDef: getDateData(0, JSON.parse(JSON.stringify(this.$t('pageQuery.queryArrData')))),
      tHide: true,
      tFrom: this.getDate(null, -10, 0, 0),
      tTo: this.getDate(null, 0, 0, 0),
      tDef: getDateData(1, JSON.parse(JSON.stringify(this.$t('pageQuery.queryArrData')))),
    };
  },
  props: {
    data: Object,
  ...
  methods: {
    ...
    getDate(time, y, m, d) {
      ...
      return this.dateFmt(ndt.getTime() + sd, 'YYYY/MM/DD');
    },
    showFrom() {
