<template>
  <v-touch class="bank-select flex-between" @tap="showFun">
    ...
    <div class="select-arrow flex-center">
      <svg width=".12rem" height=".12rem" style="width:.12rem;height:.12rem" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" version="1.1">
        <path d="M 0 14.5 L 100 14.5 L50 85.5 Z" :fill="fill"></path>
      </svg>
    </div>
  </v-touch>
</template>
...
export default {
  inheritAttrs: false,
  name: 'BankSelect',
  props: { data: Object, placeholder: String },
  computed: {
    ...mapState('app', { theme: state => state.theme }),
    fill() {
      const defColor = /black/i.test(this.theme) ? '#bababa' : '#bababa';
