<template>
  <bet-cover-box :class="`nb-bind-card-${vert ? 'vert' : 'card'}`" :index="399999" :show="show" @close="closeFun">
    <div class="bind-card-body">
      ...
      <div class="body-items-bank" v-else-if="horizScreen">
        <div class="body-items-bank-left">
        ...
        <div class="body-items-bank-right">
          <div class="bank-box flex-between">
      ...
      <v-touch class="body-btn flex-center" @tap="addCardFun">
...
export default {
    ...
    async pickChange(v) {
      if (v && !v.id && this.proCityData && `${v.title}` === `${this.proCityData.title}`) {
        if (v.select && v.select.text) {
          ...
          if (!this.pickData.hide) {
            this.setPickerData(JSON.parse(JSON.stringify(this.proCityData)));
          }
        }
      }
    },
  ...
  methods: {
    ...
    async setDefault() {
      this.proCityData = Object.assign({ hide: true, default: '', data: [] }, this.proCityData);
      this.cNameData = Object.assign({ hide: true, default: '', data: [] }, this.cNameData);
      this.typeData = Object.assign({ hide: true, default: '', data: [] }, this.typeData);
      this.queryObj = Object.assign({ bankAccountType: '', bankName: '' }, this.queryObj, {
        bankAccountNo: '',
        branchName: '',
      });
...
<style lang="less">
.nb-bind-card-vert .nb-bet-public-box-body, .nb-bind-card-card .nb-bet-public-box-body {
  top: .6rem;
  .bind-card-body {
    position: relative;
    border-radius: .1rem;
...
.horizontal .nb-bind-card-card .nb-bet-public-box-body {
  top: .1rem;
  .bind-card-body {
    width: 6.3rem;
    .body-items-bank {
      height: 2.2rem;
      .body-items-bank-left, .body-items-bank-right { width: 3.15rem; }
      .body-items-bank-left { float: left; }
      .body-items-bank-right { float: right; }
    }
  }
}
</style>
