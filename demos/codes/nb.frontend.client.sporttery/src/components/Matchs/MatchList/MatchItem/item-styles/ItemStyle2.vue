<template>
  <section class="item-style-2" :style="{ borderColor: flagColor }" >
    <v-touch class="left" @tap="toDetail" >
      <div>
        <div class="tour">
          <rolling-text :text="match.abbrName || match.tournamentName" :max-length="8" scrollamount="3" />
        </div>
        <div>
        ...
        <div v-if="period" class="period">{{$t(`common.periods.${period}`)}}</div>
        <div class="more-games" :style="{ color: '#716d6d' }">+{{match.matchGame || 0}}</div>
      </div>
    </v-touch>
    <div class="right">
      <v-touch class="teams" @tap="toDetail" >
        <div class="team">
          <rolling-text :text="match.competitor1Name" :max-length="10" scrollamount="3" />
        </div>
        <div class="score">
        ...
        <div class="team">
          <rolling-text :text="match.competitor2Name" :max-length="10" scrollamount="3" />
        </div>
      </v-touch>
...
export default {
  ...
  components: { RollingText, ListOptions },
  methods: {
    toDetail() {
