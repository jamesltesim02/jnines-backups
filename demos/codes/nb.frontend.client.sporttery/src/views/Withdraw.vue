<template>
  <list-page class="withdraw-page img-bg-page" v-if="!horizScreen">
    ...
    <div class="withdraw-body">
      ...
      <v-touch :class="`withdraw-button${actFlag ? '-active' : ''} flex-center`" @tap="submitFun">
        {{$t('withBank.submit')}}
      </v-touch>
  ...
  <div class="withdraw-page" v-else>
    <div class="withdraw-page-left img-bg-page flex-none-col">
      <div class="withdraw-left-header">
        <nav-bar :title="$t('withBank.withdraw')" slot="header" transparent />
      </div>
      <div class="withdraw-left-content">
...
export default {
  data() {
    return {
      ...
      bankData: Object.assign({ hide: true, default: '', data: [[]] }, { join: '/', title: this.$t('withBank.changeBank') }),
    };
  },
  computed: {
    ...mapState('app', {
    ...
    async submitFun() {
      if (this.actFlag) {
        ...
        if (this.sel.ver) {
          ...
          if (nRate !== oRate) {
            this.$toast(this.$t('withBank.verRateChange'));
            return;
          }
...
.withdraw-page-left {
  ...
  width: 3.75rem;
  height: 100%;
  .withdraw-left-header {
    position: relative;
    z-index: 13;
  }
  .withdraw-left-content {
    position: relative;
    flex-grow: 1;
    z-index: 10;
    width: 3.75rem;
