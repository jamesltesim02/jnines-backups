<template>
  <list-page class="query-page" v-if="!horizScreen">
    <nav-bar :title="$t('pageQuery.queryTitle')" slot="header" />
    ...
    <v-touch :class="`query-button${actFlag ? '-active' : ''} flex-center`" @tap="submitFun">
      {{$t('pageQuery.submit')}}
    </v-touch>
    <p class="query-alert flex-center">{{$t('pageQuery.queryAlert')}}</p>
  </list-page>
  <div class="query-page" v-else>
    <div class="query-page-left flex-none-col">
      <div class="query-left-header">
        <nav-bar :title="$t('pageQuery.queryTitle')" />
      </div>
      <div class="query-left-content">
        <query-select class="query-type-select" :data="typeData" :placeholder="$t('pageQuery.queryType')" />
        <date-select class="query-date-select" :data="dateData" @change="changeDateFun" />
      </div>
    </div>
    <div class="query-page-right flex-center-col">
      <v-touch :class="`query-button${actFlag ? '-active' : ''} flex-center`" @tap="submitFun">
        {{$t('pageQuery.submit')}}
      </v-touch>
      <p class="query-alert flex-center">{{$t('pageQuery.queryAlert')}}</p>
...
export default {
  data() {
    return {
      ...
      typeData: getTypeData(this.$t('pageQuery.queryType'), [[
        ...
        { value: '6', text: this.$t('pageQuery.promotion') },
        { value: '9', text: this.$t('pageQuery.betting') },
      ]], JSON.parse(JSON.stringify(this.$t('pageQuery.queryArrData')))),
    };
  },
  computed: {
    ...mapState('query', {
  ...
  methods: {
    ...mapMutations('query', ['setQueryData', 'setStatsData']),
    checkAct(v) {
    ...
    changeDateFun(from, to) {
      ...
      this.checkAct(dt);
      this.setQueryData(dt);
      this.setStatsData(JSON.parse(JSON.stringify(this.$t('pageQuery.queryArrData'))));
    },
    submitFun() {
      if (this.actFlag) {
